...
try:
    ...
    # Fallback: try relative imports when running from project root
    import sys, os
...
class AMDWindowsProvider(BaseGPUProvider):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # so that super().__init__() can override them via property setters
        ...
        # Call super().__init__() LAST so it can override instance attributes
        ...
        try:
            ...
            for proc in psutil.process_iter(['pid', 'name', 'status']):
                try:
                    ...
                    if proc_name == target_process or target_process in proc_name:
                        found = True
        ...
        if self._wmi_available and 'wmi' in self._active_sources:
            try:
                ...
                timeout_result = execute_with_timeout(
                    ...
                    args=(device_id,),
                    timeout=self._wmi_timeout,
                    default={},
                )

                if timeout_result.success and timeout_result.data:
                    wmi_data = timeout_result.data
        ...
        if self._pytorch_available and 'pytorch' in self._active_sources:
            try:
                timeout_result = execute_with_timeout(
                    ...
                    args=(device_id,),
                    timeout=self._collection_timeout,
                    default={},
                )

                if timeout_result.success and timeout_result.data:
                    pytorch_data = timeout_result.data
                else:
